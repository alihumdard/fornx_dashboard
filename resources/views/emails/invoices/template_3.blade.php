...
.invoice-right {
    display: table-cell;
    vertical-align: top;
    width: 50%;  /* Fix width so it doesn’t stretch oddly */
}
    ...
    <div class="invoice-container">
            ...
            <div class="invoice-left">
                ...
                <div class="billed-to">
                    ...
                    <div class="billed-details">
                        {{ $invoice->client_city ?? 'Los Angeles' }},
                        {{ $invoice->client_state ?? 'CA' }}
                        {{ $invoice->client_zip ?? '90001' }}
                    </div>
                    <div class="billed-details">{{ $invoice->client_country ?? 'United States' }}</div>
                </div>
            </div>

            <div class="invoice-right company-info">
                @if($pdf_logo)
        ...
        <div class="billed-section">
            <div class="invoice-info">
                <div class="invoice-details"><span class="invoice-number">Invoice #:</span>
                    {{ $invoice->invoice_number ?? 'INV-2023-001' }}</div>
                <div class="invoice-details"><span class="invoice-number">Date:</span>
                    {{ $invoice->invoice_date ? \Carbon\Carbon::parse($invoice->invoice_date)->format('d M, Y') : '' }}
                </div>
                <div class="invoice-details"><span class="invoice-number">Due Date:</span>
                    {{ $invoice->due_date ? \Carbon\Carbon::parse($invoice->due_date)->format('d M, Y') : '' }}</div>
            </div>
        </div>
        ...
        <div class="payment-section">
            <div class="payment-details">{{ $invoice->terms ?? 'Please pay within 15 days of receiving this invoice.' }}
            </div>
        </div>

        <!-- Footer -->
        <div class="invoice-footer">
            <div>{{ $invoice->website ?? 'www.fronxsolution.com' }} | {{ $invoice->email ?? 'hello@fronxsolution.com' }}
                | {{ $invoice->phone_number ?? '+1 (555) 123-4567' }}</div>
        </div>
    </div>
</body>
